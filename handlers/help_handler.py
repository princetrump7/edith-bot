...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    msg = (
        ...
        f"📝 **Analyze** — `/summarize`, `/translate`, `/grammar`, `/sentiment`\n"
        f"💻 **Code** — `/explain`, `/debug`, `/format`\n"
        f"📊 **Utilities** — `/wordcount`, `/time`\n"
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    msg = (
        ...
        f"`/sentiment <text>` — Analyze sentiment\n"
        f"`/wordcount <text>` — Word & character count\n"
        f"`/extracturls <text>` — Extract URLs\n\n"
